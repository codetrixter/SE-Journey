...
Follow-up: Can you implement the queue such that each operation is amortized O(1) time complexity?
 In other words, performing n operations will take overall O(n) time even if one of those operations may
  take longer.
*/
...
/// @brief Trivial impl, with O(n) complexity with each push or pop
/* class MyQueue
{
private:
    stack<int> s1, s2;
...
/// @brief //TODO: Amortized O(n) soliution
class MyQueue
{
private:
    stack<int> pshstk, popstk;
...
public:
    ...
    bool empty()
    {
        return pshstk.empty() && popstk.empty();
    }
};
