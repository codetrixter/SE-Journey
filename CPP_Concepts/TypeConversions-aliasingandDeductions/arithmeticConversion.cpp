 ...
 * @author Abhishek
 * @brief Here we discuss the type deduction in case if there are multiple types used as part of an expression.
 ...
 * 2- There are two rules followed while doing arithmetic conversions:
    ...
    converted to the type of the operand with highr priority,
...
int main(int argc, char const *argv[])
{
    int i{2};
    double d{3.5};
    //the expression is of type double printed as d 5.5
    std::cout << typeid(i+d).name() << " " << i + d << '\n';

    //***************************
    short a{4};
    short b{5};
    //since both the types are not on the priority list hence they are promoted to integer which is printed as i 9
    std::cout << typeid(a+b).name() << " " << a+b << "\n";

    //***************************
    //Here since unsigned int has more priority than int so exp. is converted to unsigned int, but (-5) is out of range
    //for uint, hence circled value is printed as a result. o/p = j 4294967291
    std::cout << typeid(5u+10).name() << " " << 5u - 10 << "\n";
    
    //here expected output is true but the actual output is false, because of the above reason.
    //o/p = b false, since -3 is converted to unsigned int which is a large value and hence the result is false.
